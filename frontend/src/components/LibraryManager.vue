...
import { api } from '../api'
import type { Collection, MediaItem, MediaPage, ScanJob, Tag } from '../types'
import DownloadManager from './DownloadManager.vue'

const props = defineProps<{ watchlistIds: string[] }>()
const emit = defineEmits<{ close: []; changed: []; watchlistChanged: [] }>()
...
const section = ref<'media' | 'downloads'>('media')
...
async function deleteSource(item: MediaItem) {
  ...
  await api.deleteSource(item.id); detail.value = null; await load(page.value.page); emit('changed'); emit('watchlistChanged')
}
...
<template>
  <section class="manager">
    ...
    <div class="manager-body">
      ...
      <DownloadManager v-else embedded @media-changed="emit('changed')" @watchlist-changed="emit('watchlistChanged')" />
    </div>
    <form v-if="detail" class="detail-panel" @submit.prevent="saveDetail"><header><strong>视频详情</strong><button type="button" @click="detail=null">×</button></header><label>标题<input v-model.trim="detail.name" required></label><label>备注<textarea v-model="detail.notes" rows="5"></textarea></label><label>评分<select v-model="detail.rating"><option :value="null">未评分</option><option v-for="n in 5" :key="n" :value="n">{{ n }} 星</option></select></label><label><input v-model="detail.favorite" type="checkbox"> 收藏</label><label><input v-model="detail.completed" type="checkbox"> 已看完</label><p>{{ detail.location }}<br>{{ detail.video_codec || '—' }} / {{ detail.audio_codec || '—' }}</p><button class="primary">保存修改</button><button v-if="detail.kind !== 'local'" type="button" @click="editSourceFromDetail">编辑网络源</button><button v-if="detail.kind !== 'local'" type="button" class="danger-button" @click="deleteSource(detail)">删除网络源</button></form>
